...
class enrol_elediacohortgreeting_handler {
    ...
    public static function member_added(\core\event\cohort_member_added $event) {
        ...
        foreach ($instances as $instance) {
            ...
            eledia_coursegreeting ($event->relateduserid, $instance);
            ...
            if ($instance->customint2) {
                if (!groups_is_member($instance->customint2, $event->relateduserid)) {
                    if ($group = $DB->get_record('groups', array('id'=>$instance->customint2, 'courseid'=>$instance->courseid))) {
                        groups_add_member($group->id, $event->relateduserid, 'enrol_elediacohortgreeting', $instance->id);
                    }
                }
            }
        }
    ...
    public static function member_removed(\core\event\cohort_member_removed $event) {
        ...
        if (!$instances = $DB->get_records('enrol', array('customint1'=>$event->objectid, 'enrol'=>'elediacohortgreeting'), 'id ASC')) {
            return true;
        }
 ...
 */
function enrol_elediacohortgreeting_sync(progress_trace $trace, $courseid = NULL) {
    global $CFG, $DB;
    require_once("$CFG->dirroot/group/lib.php");
    ...
    // Finally sync groups.
    $affectedusers = groups_sync_with_enrolment('elediacohortgreeting', $courseid);
