 ...
 */
class enrol_elediacohortgreeting_handler {
    /**
    ...
    public static function member_added($ca) {
        ...
        $plugin = enrol_get_plugin('elediacohortgreeting');
        foreach ($instances as $instance) {
            ...
            eledia_coursegreeting ($ca->userid, $instance);
            ...
            if ($instance->customint2) {
                if (!groups_is_member($instance->customint2, $ca->userid)) {
                    if ($group = $DB->get_record('groups', array('id'=>$instance->customint2, 'courseid'=>$instance->courseid))) {
                        groups_add_member($group->id, $ca->userid, 'enrol_elediacohortgreeting', $instance->id);
                    }
                }
            }
        }
    ...
    public static function member_removed($ca) {
        ...
        if (!$instances = $DB->get_records('enrol', array('customint1'=>$ca->cohortid, 'enrol'=>'elediacohortgreeting'), 'id ASC')) {
            return true;
        }
        ...
        foreach ($instances as $instance) {
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($ue->status != ENROL_USER_SUSPENDED) {
                    ...
                    role_unassign_all(array('userid'=>$ue->userid, 'contextid'=>$context->id, 'component'=>'enrol_elediacohortgreeting', 'itemid'=>$instance->id));
                }
            }
        }
    ...
    public static function deleted($cohort) {
        ...
        // Does anything want to sync with this cohort?
        if (!$instances = $DB->get_records('enrol', array('customint1'=>$cohort->id, 'enrol'=>'elediacohortgreeting'), 'id ASC')) {
            return true;
        }
        ...
        foreach ($instances as $instance) {
            if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                $context = context_course::instance($instance->courseid);
                role_unassign_all(array('contextid'=>$context->id, 'component'=>'enrol_elediacohortgreeting', 'itemid'=>$instance->id));
 ...
 */
function enrol_elediacohortgreeting_sync($courseid = NULL, $verbose = false) {
    global $CFG, $DB;
    require_once("$CFG->dirroot/group/lib.php");
    ...
    foreach($rs as $ue) {
        ...
        if ($ue->status == ENROL_USER_SUSPENDED) {
            ...
            eledia_coursegreeting ($ue->userid, $instance);
            if ($verbose) {
    ...
    $sql = "SELECT e.roleid, ue.userid, c.id AS contextid, e.id AS itemid, e.courseid
              FROM {user_enrolments} ue
              JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'elediacohortgreeting' AND e.status = :statusenabled $onecourse)
              JOIN {role} r ON (r.id = e.roleid)
              JOIN {context} c ON (c.instanceid = e.courseid AND c.contextlevel = :coursecontext)
         LEFT JOIN {role_assignments} ra ON (ra.contextid = c.id AND ra.userid = ue.userid AND ra.itemid = e.id AND ra.component = 'enrol_elediacohortgreeting' AND e.roleid = ra.roleid)
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ra) {
        role_assign($ra->roleid, $ra->userid, $ra->contextid, 'enrol_elediacohortgreeting', $ra->itemid);
        if ($verbose) {
    ...
    $sql = "SELECT ra.roleid, ra.userid, ra.contextid, ra.itemid, e.courseid
              ...
              JOIN {enrol} e ON (e.id = ra.itemid AND e.enrol = 'elediacohortgreeting' $onecourse)
         LEFT JOIN {user_enrolments} ue ON (ue.enrolid = e.id AND ue.userid = ra.userid AND ue.status = :useractive)
             WHERE ra.component = 'enrol_elediacohortgreeting' AND (ue.id IS NULL OR e.status <> :statusenabled)";
    $params = array();
    $params['statusenabled'] = ENROL_INSTANCE_ENABLED;
    $params['useractive'] = ENROL_USER_ACTIVE;
    $params['coursecontext'] = CONTEXT_COURSE;
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ra) {
        role_unassign($ra->roleid, $ra->userid, $ra->contextid, 'enrol_elediacohortgreeting', $ra->itemid);
        if ($verbose) {
    ...
    $sql = "SELECT gm.*, e.courseid, g.name AS groupname
              ...
              JOIN {enrol} e ON (e.enrol = 'elediacohortgreeting' AND e.courseid = g.courseid $onecourse)
              JOIN {user_enrolments} ue ON (ue.userid = gm.userid AND ue.enrolid = e.id)
             WHERE gm.component='enrol_elediacohortgreeting' AND gm.itemid = e.id AND g.id <> e.customint2";
    $params = array();
    $params['courseid'] = $courseid;
    ...
    $rs = $DB->get_recordset_sql($sql, $params);
    foreach($rs as $ue) {
        groups_add_member($ue->groupid, $ue->userid, 'enrol_elediacohortgreeting', $ue->enrolid);
        if ($verbose) {
 ...
 * @return int
 */
function enrol_elediacohortgreeting_enrol_all_users(course_enrolment_manager $manager, $cohortid, $roleid) {
    global $DB;
    ...
    foreach ($rs as $user) {
        ...
        eledia_coursegreeting ($user->userid, $instance);
    }
    $rs->close();
    return $count;
}
 ...
 * @return array
 */
function enrol_elediacohortgreeting_get_cohorts(course_enrolment_manager $manager) {
    global $DB;
    $context = $manager->get_context();
    $cohorts = array();
    $instances = $manager->get_enrolment_instances();
    $enrolled = array();
    foreach ($instances as $instance) {
        if ($instance->enrol == 'elediacohortgreeting') {
 ...
 * @return boolean
 */
function enrol_elediacohortgreeting_can_view_cohort($cohortid) {
    global $DB;
 ...
 */
function enrol_elediacohortgreeting_search_cohorts(course_enrolment_manager $manager, $offset = 0, $limit = 25, $search = '') {
    global $DB;
    $context = $manager->get_context();
    $cohorts = array();
    $instances = $manager->get_enrolment_instances();
...
function eledia_coursegreeting ($userid, $instance) {
    ...
    $messagetext = $instance->customtext1;

    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
    email_to_user($user, $support_user, $subject, $messagetext);
}