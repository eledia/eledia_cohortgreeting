...
$PAGE->set_url(new moodle_url('/enrol/elediacohortgreeting/ajax.php', array('id'=>$id, 'action'=>$action)));
...
switch ($action) {
    ...
    case 'getdefaultcohortrole': //TODO: use in ajax UI MDL-24280
        $cohortenrol = enrol_get_plugin('elediacohortgreeting');
    ...
    case 'getcohorts':
        ...
        $outcome->response = enrol_elediacohortgreeting_search_cohorts($manager, $offset, 25, $search);
        break;
    case 'enrolcohort':
        require_capability('moodle/course:enrolconfig', $context);
        ...
        if (!enrol_elediacohortgreeting_can_view_cohort($cohortid) || !array_key_exists($roleid, $roles)) {
            throw new enrol_ajax_exception('errorenrolcohort');
        }
        $enrol = enrol_get_plugin('elediacohortgreeting');
        $enrol->add_instance($manager->get_course(), array('customint1' => $cohortid, 'roleid' => $roleid));
        enrol_elediacohortgreeting_sync($manager->get_course()->id);
        break;
    case 'enrolcohortusers':
        ...
        $result = enrol_elediacohortgreeting_enrol_all_users($manager, $cohortid, $roleid);
        if ($result === false) {
