...
/**
 * elediacohortgreeting enrolment plugin.
 *
 ...
 */
define('ELEDIACOHORTGREETING_CREATE_GROUP', -1);

/**
 * elediacohortgreeting enrolment plugin implementation.
 * @author Petr Skoda
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_elediacohortgreeting_plugin extends enrol_plugin {
    ...
    public function can_delete_instance($instance) {
        ...
        return has_capability('enrol/elediacohortgreeting:config', $context);
    }
    ...
    public function add_instance($course, array $fields = null) {
        ...
        if (!empty($fields['customint2']) && $fields['customint2'] == ELEDIACOHORTGREETING_CREATE_GROUP) {
            // Create a new group for the cohort if requested.
            $context = context_course::instance($course->id);
            require_capability('moodle/course:managegroups', $context);
            $groupid = enrol_elediacohortgreeting_create_new_group($course->id, $fields['customint1']);
        ...
        enrol_elediacohortgreeting_sync($trace, $course->id);
        $trace->finished();
    ...
    public function update_instance($instance, $data) {
        ...
        if ($data->roleid != $instance->roleid) {
            ...
            $params = array(
                ...
                'component' => 'enrol_elediacohortgreeting',
                'itemid' => $instance->id
            );
            role_unassign_all($params);
        }
        // Create a new group for the cohort if requested.
        if ($data->customint2 == ELEDIACOHORTGREETING_CREATE_GROUP) {
            require_capability('moodle/course:managegroups', $context);
            $groupid = enrol_elediacohortgreeting_create_new_group($instance->courseid, $data->customint1);
        ...
        require_once("$CFG->dirroot/enrol/elediacohortgreeting/locallib.php");
        $trace = new null_progress_trace();
        enrol_elediacohortgreeting_sync($trace, $instance->courseid);
        $trace->finished();
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol_user($instance, $ue) && has_capability('enrol/elediacohortgreeting:unenrol', $context)) {
    ...
    protected function get_group_options($coursecontext) {
        ...
        if (has_capability('moodle/course:managegroups', $coursecontext)) {
            $groups[ELEDIACOHORTGREETING_CREATE_GROUP] = get_string('creategroup', 'enrol_elediacohortgreeting');
        }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        $sql = "roleid = :roleid AND customint1 = :customint1 AND courseid = :courseid AND enrol = 'elediacohortgreeting' AND id <> :id";
 ...
 * @return bool
 */
function enrol_elediacohortgreeting_allow_group_member_remove($itemid, $groupid, $userid) {
    return false;
}

/**
 * Create a new group with the elediacohortgreetings name.
 *
 * @param int $courseid
 * @param int $cohortid
 * @return int $groupid Group ID for this cohort.
 */
function enrol_elediacohortgreeting_create_new_group($courseid, $cohortid) {
    global $DB, $CFG;
    ...
    $a->name = $groupname;
    $a->increment = '';
    $groupname = trim(get_string('defaultgroupnametext', 'enrol_elediacohortgreeting', $a));
    $inc = 1;
    ...
    while ($DB->record_exists('groups', array('name' => $groupname, 'courseid' => $courseid))) {
        $a->increment = '(' . (++$inc) . ')';
        $newshortname = trim(get_string('defaultgroupnametext', 'enrol_elediacohortgreeting', $a));
