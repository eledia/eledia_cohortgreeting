YUI.add('moodle-enrol_elediacohortgreeting-quickenrolment', function(Y) {

    var CONTROLLERNAME = 'Quick cohort enrolment controller',
        COHORTNAME = 'elediacohortgreeting',
        COHORTID = 'cohortid',
        ENROLLED = 'enrolled',
        NAME = 'name',
        USERS = 'users',
    ...
    CONTROLLER.prototype = {
        initializer : function(config) {
            ...
            var base = Y.Node.create('<div class="'+CSS.PANELCONTENT+'"></div>')
                ...
                .append(Y.Node.create('<div class="'+CSS.FOOTER+'"></div>')
                    .append(Y.Node.create('<div class="'+CSS.SEARCH+'"><label for="enrolcohortsearch">'+M.str.enrol_elediacohortgreeting.cohortsearch+':</label></div>')
            ...
            Y.all('.enrol_elediacohortgreeting_plugin input').each(function(node){
        ...
        show : function(e) {
            ...
            var rolesselect = Y.one('#id_enrol_elediacohortgreeting_assignable_roles');
            if (rolesselect) {
                rolesselect.focus();
            }
        },
            ...
            switch (e.type.replace(/^[^:]+:/, '')) {
                case 'cohortsloaded' :
                    ...
                    if (this.get(MORERESULTS)) {
                        var fetchmore = Y.Node.create('<div class="'+CSS.MORERESULTS+'"><a href="#">'+M.str.enrol_elediacohortgreeting.ajaxmore+'</a></div>');
                ...
                case 'assignablerolesloaded':
                    ...
                    content = Y.Node.create('<select id="id_enrol_elediacohortgreeting_assignable_roles"></select>');
                    for (i in roles) {
                        content.append(Y.Node.create('<option value="'+i+'">'+roles[i]+'</option>'));
                    }
                    panel.get('contentBox').one('.'+CSS.PANELROLES).setContent(Y.Node.create('<div><label for="id_enrol_elediacohortgreeting_assignable_roles">'+M.str.role.assignroles+':</label></div>').append(content));

                    this.getDefaultCohortRole();
                    Y.one('#id_enrol_elediacohortgreeting_assignable_roles').focus();
                    break;
    ...
    M.enrol_elediacohortgreeting = M.enrol || {};
    M.enrol_elediacohortgreeting.quickenrolment = {
        init : function(cfg) {
            new CONTROLLER(cfg);
        }
    }
